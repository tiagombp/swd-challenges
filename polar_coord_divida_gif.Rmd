...
animate(plot_din + ease_aes('quadratic-in-out'), height = 480, width = 800)
...
```{r}
plot_dinamico <- plot_equalizador + 
  theme(panel.grid.major.y = element_line(color = "grey90", linetype = "dotted")) +
  labs(subtitle = "{frame_time}") +
  transition_time(Ano) +
  ease_aes('linear')
...
```{r}
  ...
  scale_x_continuous(labels = c("Janeiro", "Fevereiro", "Mar̤o", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"), breaks = 1:12) +
  labs(
    x = NULL,
    y = NULL
  ) + 
