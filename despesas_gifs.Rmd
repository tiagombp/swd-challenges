...
tema <- function(){
    ...
    theme(
      ...
      axis.text = element_text(family = "Source Sans Pro", colour = "grey20", size = 16),
...
```{r}
Category <- c("Personnel and Public Sector Pensions","Healthcare Costs","Education Costs","Welfare Costs","Administrative Costs","Pension Benefits (private sector)","Investments","Transfers to States and Municipalities")

Categoria_sem_acento <- c("Pessoal (Ativo, Inativo, Pension.)", "Custeio da Saude", "Custeio da Educacao", "Custeio Social", "Custeio Administrativo", "Beneficios Previdenciarios RGPS", "Investimentos", "Transferencias a Estados e Mun.")

Categoria_reduzida <- c("Pessoal", "Custeio Saúde", "Custeio Educação", "Custeio Social", "Custeio Administrativo", "Beneficios RGPS", "Investimentos", "Transferências")
...
plot_estatico <- ggplot(dados_plot, aes(y = Valor, x = reorder(Categoria_reduzida, Valor))) +
  ...
  geom_text(aes(y = Valor + 0.005, 
            ...
            size = 6, family = "Open Sans", vjust = 0) +
...
```{r}
graf_linhas <- ggplot(dados_plot, aes(x = Ano, y = Valor, group = Categoria_sem_acento, color = CoresEscuras)) +
  geom_line(size = 1) +
...
```{r}
 gif_linhas <- graf_linhas + transition_reveal(Categoria_sem_acento, Ano)
